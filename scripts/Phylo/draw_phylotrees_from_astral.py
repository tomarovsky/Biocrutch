...
if __name__ == '__main__':
    ...
    group_additional.add_argument(
        ...
        help='comma-separated list of constant colors per metrics',
    )
    group_additional.add_argument(
        '-c', '--color_by_value', action='store_true', default=False, help="colors per metrics (disables '--color_per_metric' option)"
    )
    group_additional.add_argument(
        '--thresholds_and_colors',
        type=lambda s: dict(zip([int(s) for i in s[::2]], s[1::2])),
        default={90: 'Green', 70: 'Purple', 50: 'Blue', 0: 'Red'},
        help="colors per metrics (disables '--color_per_metric' option)."
