...
def violin_to_poligon(ax, data, bins, x_offset, color, edgecolor, label=None):
    ...
    ax.fill_betweenx(bin_centers, x_offset, x_offset - hist, color=color, linewidth=0.4, edgecolor=edgecolor, zorder=2)
    ax.fill_betweenx(bin_centers, x_offset, x_offset + hist, color=color, linewidth=0.4, edgecolor=edgecolor, zorder=2, label=label)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=('png', 'svg'),
        help='Comma-separated list of formats (supported by matlotlib) of output figure.Default: png,svg',
    )
    ...
    parser.add_argument(
        '-b', '--bin_width', action='store', dest='bin_width', required=True, default=0.1, type=float, help='Width of bins in SNPs/1kbp'
    )
    parser.add_argument(
        '-m',
        '--multiplicator',
