...
def main():
    ...
    sns.violinplot(
        data=[df_dict[entry]['density'] for entry in df_dict], palette='turbo', scale='width', inner=None, linewidth=0, saturation=0.5
    )  # or inner="box"
    sns.boxplot(
        data=[df_dict[entry]['density'] for entry in df_dict],
        palette='turbo',
        width=0.1,
        linewidth=0.4,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--only_count', action='store_true', dest='only_count', default=False, help='Only count variants, do not draw them. Default: False'
    )
    parser.add_argument(
        '--pretty',
        action='store_true',
