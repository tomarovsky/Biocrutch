...
def main():
    ...
    if not args.species:
        species = [
            file.split("_")[-1][:-4] for file in args.full_tables
        ]  # if not args.species else args.species
    else:
        species = args.species
    for status in statuses:
        fig, ax = plt.subplots(figsize=(16, 8), nrows=1, ncols=2)
        df_status_list = [
            set(list(df.loc[df["Status"] == status]["# Busco id"])) for df in df_list
        ]
...
if __name__ == "__main__":
    ...
    group_required.add_argument(
        "-o", "--outdir", type=str, help="output directory name"
    )
