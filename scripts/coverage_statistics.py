...
def main():
    ...
    for line in args.input:
        ...
        if frame_line_counter == args.frame_size:
            frame_id += 1
            df.loc['frame_'+str(frame_id)] = frame_stats(
        ...
        if overlapping_frame_line_counter == args.frame_size:
            overlapping_frame_id += 1
            df.loc['frame_'+str(overlapping_frame_id)] = frame_stats(
    ...
    df.loc['whole_genome'] = frame_stats(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="script for calculating median, average, maximum and minimum coverage in file.bam")
